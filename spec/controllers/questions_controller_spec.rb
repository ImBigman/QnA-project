...
RSpec.describe QuestionsController, type: :controller do
  ...
  describe 'PATCH #positive_vote', format: :json do
    context 'As not an author of resource' do
      ...
      it 'vote for resource' do
        patch :positive_vote, params: { id: question, question: attributes_for(:question, user: user), format: :json }
        question.reload
      ...
      it 'render JSON response' do
        ...
        expect(response.body.to_i).to eq(question.vote_score)
      end
      ...
      describe 'You completely change your opinion' do
        let(:question4) { create(:question, user: user, vote_score: -1) }
        let!(:vote) {  create :vote, user_id: user1.id, votable: question4, score: -1 }

        scenario 'from negative to positive' do
          patch :positive_vote, params: { id: question4, question: attributes_for(:question, user: user), format: :json }
          patch :positive_vote, params: { id: question4, question: attributes_for(:question, user: user), format: :json }
          question4.reload

          expect(response.body).to_not eq('You have already voted')
          expect(question4.vote_score).to eq(1)
        end
      end
    end
  ...
  describe 'PATCH #negative_vote', format: :json do
    context 'As not an author of resource' do
      ...
      it 'vote for resource' do
        patch :negative_vote, params: { id: question, question: attributes_for(:question, user: user), format: :json }
        question.reload
      ...
      describe 'You completely change your opinion' do
        ...
        scenario 'from positive to negative' do
          patch :negative_vote, params: { id: question4, question: attributes_for(:question, user: user), format: :json }
          patch :negative_vote, params: { id: question4, question: attributes_for(:question, user: user), format: :json }
          question4.reload
